...
export const useDuplicateHandling = (onRefreshPantry: () => Promise<void>) => {
  ...
  const checkBeforeSave = useCallback(
    async (itemData: any): Promise<boolean> => {
      if (!user?.id) return false;

      console.log(
        '[useDuplicateHandling] 🔍 PRE-SAVE duplicate check for:',
        itemData.item_name,
      );
      ...
      try {
        const duplicateGroups =
          await DuplicateDetectionService.detectDuplicatesForItem(
            user.id,
            itemData.item_name,
          );

        console.log(
          '[useDuplicateHandling] 📊 PRE-SAVE duplicate check result:',
          {
        ...
        if (duplicateGroups.length > 0) {
          console.log(
            '[useDuplicateHandling] ✅ PRE-SAVE: Found duplicates, showing merge modal',
          );
          setState(prev => ({
            ...prev,
        ...
        console.log(
          '[useDuplicateHandling] ❌ PRE-SAVE: No duplicates found, can proceed with save',
        );
        setState(prev => ({ ...prev, isDetecting: false }));
        return false; // No duplicates, safe to save
      } catch (error) {
        console.error(
          '[useDuplicateHandling] Error in pre-save duplicate check:',
          error,
        );
        setState(prev => ({ ...prev, isDetecting: false }));
  ...
  const checkForDuplicates = useCallback(
    async (itemName: string) => {
      ...
      console.log(
        '[useDuplicateHandling] 🔍 POST-SAVE duplicate check for:',
        itemName,
      );
      setState(prev => ({ ...prev, isDetecting: true }));
      ...
      try {
        ...
        console.log(
          '[useDuplicateHandling] 📊 POST-SAVE duplicate check result:',
          {
            itemName,
            duplicateGroupsFound: duplicateGroups.length,
            firstGroup: duplicateGroups[0]
        ...
        if (duplicateGroups.length > 0) {
          console.log(
            '[useDuplicateHandling] ✅ POST-SAVE: Setting showMergeDialog to TRUE',
          );
          setState(prev => ({
            ...prev,
            showMergeDialog: true,
          ...
          console.log(
            '[useDuplicateHandling] ❌ POST-SAVE: No duplicates found, not showing merge modal',
          );
          setState(prev => ({ ...prev, isDetecting: false }));
        }
      } catch (error) {
        console.error(
          '[useDuplicateHandling] Error checking duplicates:',
          error,
        );
        setState(prev => ({ ...prev, isDetecting: false }));
      }
  ...
  const loadAllDuplicates = useCallback(async () => {
    ...
    try {
      const allGroups = await DuplicateDetectionService.getAllDuplicateGroups(
        user.id,
      );
      setState(prev => ({
        ...prev,
      ...
      console.error(
        '[useDuplicateHandling] Error loading all duplicates:',
        error,
      );
  ...
  const savePendingItem = useCallback(async (): Promise<{
    success: boolean;
    error?: string;
  }> => {
    ...
    try {
      ...
      console.log(
        '[useDuplicateHandling] ✅ Successfully saved pending item:',
        state.pendingItemData.item_name,
      );
  ...
  const handleDeleteFromSuggestion = useCallback(async () => {
    ...
    const { items } = state.currentDuplicateGroup;
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            const result = await DuplicateDetectionService.deleteItem(
              mostRecentItem.id,
            );
            if (result.success) {
              setState(prev => ({
  ...
  const handleKeepBothFromSuggestion = useCallback(async () => {
    console.log(
      '[useDuplicateHandling] 💾 User chose "Keep Both" - saving pending item',
    );
  ...
  const handleEditFromReview = useCallback((item: DuplicateItem) => {
    ...
    Alert.alert(
      'Edit',
      `Edit functionality for ${item.item_name} to be implemented`,
    );
  }, []);

  const handleDeleteFromReview = useCallback(
  ...
  const handleConfirmMerge = useCallback(
    async (targetUnit: string, targetLocation: string) => {
      ...
      try {
        ...
        if (state.pendingItemData && itemsToMerge.length > 0) {
          console.log(
            '[useDuplicateHandling] 🔄 Including pending item in merge',
          );
          ...
          console.log(
            '[useDuplicateHandling] 🔄 Updated items to merge with pending item:',
            {
              count: itemsToMerge.length,
              items: itemsToMerge.map(item => ({
                id: item.id,
        ...
        if (itemsToMerge.length < 2) {
          console.log(
            '[useDuplicateHandling] ❌ MERGE FAILED: Not enough items to merge',
          );
        ...
        console.log(
          '[useDuplicateHandling] 🔄 Calling DuplicateDetectionService.mergeItems...',
        );
        ...
        if (result.success) {
          console.log(
            '[useDuplicateHandling] ✅ MERGE SUCCESSFUL - Refreshing pantry...',
          );
          setState(prev => ({
            ...prev,
            showMergeDialog: false,
