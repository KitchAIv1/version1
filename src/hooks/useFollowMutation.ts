...
export const useFollowMutation = (currentUserId?: string) => {
  ...
  return useMutation<FollowMutationResult, Error, FollowMutationParams>({
    mutationFn: async ({ targetUserId, action }: FollowMutationParams) => {
      ...
      if (!currentUserId) {
        throw new Error(
          'Current user ID is required for follow/unfollow actions',
        );
      }
      ...
      console.log(
        `[useFollowMutation] Successfully ${action}ed user ${targetUserId}`,
      );
    ...
    onSuccess: (data, { targetUserId, action }) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['userActivityFeed', currentUserId],
      });
    },
...
export const useFollowStatus = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      console.log(
        `[useFollowStatus] Checking if ${currentUserId} follows ${targetUserId}`,
      );
