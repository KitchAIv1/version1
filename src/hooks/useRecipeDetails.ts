...
const cleanPantryMatchData = (pantryData: any, recipeId: string) => {
  if (!pantryData) {
    console.warn(
      `[cleanPantryMatchData] No pantry data received for recipe ${recipeId}`,
    );
    return {
      match_percentage: 0,
      matched_ingredients: [],
  ...
  try {
    ...
    console.log(
      `[cleanPantryMatchData] Cleaned pantry data for recipe ${recipeId}:`,
      {
    ...
    console.error(
      `[cleanPantryMatchData] Error cleaning pantry data for recipe ${recipeId}:`,
      error,
    );
    return {
      match_percentage: 0,
...
export const fetchRecipeDetails = async (
  ...
  console.log(
    `[fetchRecipeDetails] Fetching for recipe ${recipeId}, user ${userId}`,
  );
  ...
  if (queryClient) {
    ...
    if (feedData && Array.isArray(feedData)) {
      existingFeedData = feedData.find(
  ...
  try {
    ...
    const { data: recipeData, error: recipeError } = await supabase.rpc(
      'get_recipe_details',
      {
        p_user_id: userId,
    ...
    if (recipeError) {
      console.error(
        '[fetchRecipeDetails] Error fetching recipe details:',
        recipeError.message,
      );
    ...
    if (!recipeData) {
      console.warn(
        `[fetchRecipeDetails] No recipe data returned for recipe ${recipeId}`,
      );
    ...
    console.log(
      `[fetchRecipeDetails] 🔍 RECIPE DETAILS AUDIT for ${recipeId}:`,
      {
    ...
    console.log(
      `[fetchRecipeDetails] 🚨 RAW RECIPE DETAILS DATA for ${recipeId}:`,
      {
    ...
    console.log(
      `[fetchRecipeDetails] 🔧 CORRECT FIELD MAPPING for ${recipeId}:`,
      {
    ...
    if (userId) {
      const { data: pantryData, error: pantryError } = await supabase.rpc(
      ...
      if (pantryError) {
        console.error(
          '[fetchRecipeDetails] Error fetching pantry match:',
          pantryError.message,
        );
        ...
        pantryMatchData = cleanedPantryData || {
          match_percentage: 0,
          matched_ingredients: [],
          missing_ingredients: [],
        };
    ...
    console.log(
      `[fetchRecipeDetails] Pantry match data for recipe ${recipeId}:`,
      {
        ...
        total_count:
          (pantryMatchData.matched_ingredients?.length || 0) +
          (pantryMatchData.missing_ingredients?.length || 0),
      },
    );
    ...
    if (
      combinedData.is_liked_by_user === undefined ||
      combinedData.likes === undefined
    ) {
      console.warn(
        `[fetchRecipeDetails] Like fields missing from get_recipe_details RPC for recipe ${recipeId}. This should now be resolved.`,
      );
    ...
    return await processRecipeDetailsData(
      combinedData,
      recipeId,
      userId,
      existingFeedData,
...
const fetchRecipeDetailsViaFallback = async (
  recipeId: string,
  userId?: string,
  existingFeedData?: any,
) => {
  console.log(
    `[fetchRecipeDetailsViaFallback] Using fallback method for recipe ${recipeId}`,
  );
  ...
  try {
    ...
    if (basicError || !basicRecipe) {
      console.error(
        '[fetchRecipeDetailsViaFallback] Basic recipe query failed:',
        basicError,
      );
      throw new Error('Recipe not found');
    }
    ...
    const userInteractions = { is_liked: false, is_saved: false };
    let likesCount = 0;
    let commentsCount = 0;
    ...
    if (userId) {
      ...
      if (interactions) {
        userInteractions.is_liked = interactions.some(
    ...
    if (existingFeedData) {
      ...
      commentsCount = Math.max(
        commentsCount,
        existingFeedData.commentsCount || 0,
      );
      userInteractions.is_liked =
        existingFeedData.liked ?? userInteractions.is_liked;
      userInteractions.is_saved =
        existingFeedData.saved ?? userInteractions.is_saved;
    }
    ...
    console.log(
      `[fetchRecipeDetailsViaFallback] Successfully constructed fallback data for recipe ${recipeId}`,
    );
    return await processRecipeDetailsData(
      fallbackData,
      recipeId,
    ...
    console.error(
      '[fetchRecipeDetailsViaFallback] Fallback method also failed:',
      fallbackError,
    );
...
const processRecipeDetailsData = async (
  data: any,
  recipeId: string,
  userId?: string,
  existingFeedData?: any,
  ...
  if (existingFeedData && existingFeedData.likes !== data.likes) {
    console.warn(
      `[processRecipeDetailsData] LIKE COUNT DISCREPANCY for recipe ${recipeId}:`,
      {
    ...
    if (existingFeedData.likes > data.likes) {
      console.log(
        `[processRecipeDetailsData] Using feed like count (${existingFeedData.likes}) instead of recipe details (${data.likes})`,
      );
  ...
  const cleanedSteps = (data.preparation_steps || []).map((step: string) =>
    step.replace(/^"+|"+$/g, '').trim(),
  );
...
export const prefetchRecipeDetails = async (
  queryClient: any,
  recipeId: string,
  userId?: string,
) => {
...
export const useRecipeDetails = (
  ...
  useEffect(() => {
    if (recipeId && userId) {
      const logView = async () => {
        try {
          console.log(
            `[useRecipeDetails] Logging view for recipe ${recipeId}, user ${userId}`,
          );
          ...
          if (logError) {
            ...
            if (logError.code === '23505') {
              console.log(
                `[useRecipeDetails] View already logged for recipe ${recipeId}, user ${userId} - skipping`,
              );
              return; // Don't treat duplicate views as errors
            }
            console.error(
              '[useRecipeDetails] Error calling log_recipe_view:',
              logError,
            );
            // Don't throw here to avoid breaking the main data fetch if logging fails
          } else {
            console.log(
              `[useRecipeDetails] View logged successfully for recipe ${recipeId}. Updating views count only.`,
            );
  ...
  useEffect(() => {
    ...
    if (feedData && recipeId && userId && recipeResult.data) {
      ...
      if (feedItem) {
        ...
        if (
          feedLiked !== currentRecipeData.is_liked_by_user ||
          ...
          console.log(
            `[useRecipeDetails] Reverse sync: updating recipe details from feed for recipe ${recipeId}:`,
            {
  ...
  const isLoading =
    recipeResult.isLoading || (!!userId && pantryResult.isLoading);
