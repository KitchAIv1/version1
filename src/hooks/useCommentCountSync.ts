...
import { fetchCommentCount, fetchMultipleCommentCounts } from './useRecipeComments';

export const useCommentCountSync = () => {
  const queryClient = useQueryClient();

  // Efficiently sync a single recipe's comment count
  const syncSingleRecipe = useCallback(async (recipeId: string, userId?: string) => {
    if (!recipeId) return;

    console.log(`[useCommentCountSync] 🎯 Syncing comment count for recipe ${recipeId}`);
    ...
    try {
      ...
      if (userId) {
        queryClient.setQueryData(['recipeDetails', recipeId, userId], (oldData: any) => {
          ...
          if (oldCount !== actualCount) {
            console.log(`[useCommentCountSync] 📊 Recipe ${recipeId} count: ${oldCount} → ${actualCount}`);
      ...
      queryClient.setQueryData(['feed'], (oldFeedData: any) => {
        ...
        return oldFeedData.map((item: any) => {
          if (item.id === recipeId || item.recipe_id === recipeId) {
            ...
            if (oldCount !== actualCount) {
              console.log(`[useCommentCountSync] 📊 Feed item ${recipeId} count: ${oldCount} → ${actualCount}`);
      ...
      console.error(`[useCommentCountSync] Error syncing recipe ${recipeId}:`, error);
      return 0;
    }
  }, [queryClient]);

  // Efficiently sync multiple recipes' comment counts (batch operation)
  const syncMultipleRecipes = useCallback(async (recipeIds: string[], userId?: string) => {
    if (!recipeIds.length) return {};

    console.log(`[useCommentCountSync] 🎯 Batch syncing ${recipeIds.length} recipes`);
    ...
    try {
      ...
      Object.entries(updatedCounts).forEach(([recipeId, counts]) => {
        console.log(`[useCommentCountSync] 📊 Recipe ${recipeId} count: ${counts.old} → ${counts.new}`);
      });
  ...
  const smartSync = useCallback(async (recipeIds: string[], userId?: string) => {
    if (!recipeIds.length) return;

    console.log(`[useCommentCountSync] 🧠 Smart syncing ${recipeIds.length} recipes`);
    ...
    if (feedData && Array.isArray(feedData)) {
      recipeIds.forEach(recipeId => {
        const feedItem = feedData.find((item: any) => 
          item.id === recipeId || item.recipe_id === recipeId
        );
        
        if (userId) {
          const recipeDetails = queryClient.getQueryData(['recipeDetails', recipeId, userId]) as any;
          ...
          if (feedCount !== detailsCount) {
            console.log(`[useCommentCountSync] 🔍 Discrepancy in ${recipeId}: feed=${feedCount}, details=${detailsCount}`);
    ...
    if (recipesNeedingSync.length > 0) {
      console.log(`[useCommentCountSync] 🎯 Smart sync identified ${recipesNeedingSync.length} recipes needing sync`);
      return await syncMultipleRecipes(recipesNeedingSync, userId);
    } else {
      console.log('[useCommentCountSync] ✅ Smart sync: all recipes already in sync');
      return {};
    }
