...
export const useLikeMutation = (userId?: string) => {
  ...
  return useMutation<any, Error, string, MutationContext>({
    mutationFn: async (recipeId: string) => {
      if (!userId)
        throw new Error(
          'User not authenticated. Please log in to like recipes.',
        );

      console.log(
        '[useLikeMutation] Attempting to toggle like for recipe:',
        recipeId,
        'by user:',
        userId,
      );
      ...
      console.log(
        '[useLikeMutation] Like toggle successful for recipe:',
        recipeId,
      );
    ...
    onMutate: async recipeId => {
      ...
      const { newLiked, newLikes } = cacheManager.optimisticLikeUpdate(
        recipeId,
        userId,
      );

      console.log(
        `[useLikeMutation] Optimistic update for recipe ${recipeId}:`,
        {
          newLiked,
          newLikes,
          userId,
    ...
    onSuccess: (result, recipeId, context) => {
      console.log(
        '[useLikeMutation] Like mutation successful, result:',
        result,
      );

      // Update caches with actual server response
      const serverResponseApplied = cacheManager.handleLikeServerResponse(
        recipeId,
        result?.rpcData,
        userId,
      );
      ...
      if (serverResponseApplied) {
        console.log(
          `[useLikeMutation] Applied server response for recipe ${recipeId}`,
        );
      } else {
        console.warn(
          `[useLikeMutation] Invalid server response for recipe ${recipeId}:`,
          result,
        );
      }
    },
    onError: (error, recipeId, context) => {
      ...
      Alert.alert(
        'Error',
        error.message || 'Could not update like status. Please try again.',
      );
    },
  });
};
...
export const useSaveMutation = (userId?: string) => {
  ...
  return useMutation<any, Error, string, MutationContext>({
    mutationFn: async (recipeId: string) => {
      if (!userId)
        throw new Error(
          'User not authenticated. Please log in to save recipes.',
        );

      console.log(
        '[useSaveMutation] Attempting to toggle save for recipe:',
        recipeId,
        'by user:',
        userId,
      );
      ...
      console.log(
        '[useSaveMutation] Save toggle successful for recipe:',
        recipeId,
      );
    ...
    onMutate: async recipeId => {
      ...
      console.log(
        `[useSaveMutation] Optimistic save update for recipe ${recipeId}:`,
        {
          newSaved,
          userId,
        },
    ...
    onSuccess: (result, recipeId, context) => {
      console.log(
        '[useSaveMutation] Save mutation successful, result:',
        result,
      );
      ...
      console.log(
        `[useSaveMutation] Save toggle completed for recipe ${recipeId}`,
      );
    },
    onError: (error, recipeId, context) => {
      ...
      Alert.alert(
        'Error',
        error.message || 'Could not update save status. Please try again.',
      );
    },
  });
};
...
export const useCommentMutation = (recipeId: string, userId?: string) => {
  ...
  return useMutation<any, Error, string, MutationContext>({
    mutationFn: async (commentText: string) => {
      ...
      console.log(
        '[useCommentMutation] Posting comment for recipe:',
        recipeId,
        'by user:',
        userId,
      );
      ...
      console.log(
        '[useCommentMutation] Comment posted successfully for recipe:',
        recipeId,
      );
    ...
    onMutate: async commentText => {
      ...
      const feedData = cacheManager.queryClient?.getQueryData([
        'feed',
      ]) as any[];
      const recipeData = userId
        ? (cacheManager.queryClient?.getQueryData([
            'recipeDetails',
            recipeId,
            userId,
          ]) as any)
      ...
      console.log(
        `[useCommentMutation] Optimistic comment count update for recipe ${recipeId}:`,
        {
