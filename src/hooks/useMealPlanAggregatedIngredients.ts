...
export const getIngredientsForMealPlanRange = async (
  ...
  console.log(
    `[getIngredientsForMealPlanRange] Attempting to get ingredients for meal plan from ${format(startDate, 'yyyy-MM-dd')} to ${format(endDate, 'yyyy-MM-dd')} for user ${userId}`,
  );

  try {
    // 1. Fetch meal plan items for the range
    console.log(
      '[getIngredientsForMealPlanRange] Step 1: Fetching meal plan items...',
    );
    ...
    if (planError) {
      console.error(
        '[getIngredientsForMealPlanRange] CRITICAL: Error fetching meal plan items:',
    ...
    console.log(
      '[getIngredientsForMealPlanRange] Successfully fetched planItems. Raw data:',
      JSON.stringify(planItems, null, 2),
    );

    if (!planItems || planItems.length === 0) {
      console.log(
        '[getIngredientsForMealPlanRange] No plan items found for this date range. Returning empty array.',
      );
      return [];
    }
    ...
    console.log(
      '[getIngredientsForMealPlanRange] Step 2: Processing planItems to get unique recipe IDs...',
    );
    const recipeIdToNameMap = new Map<string, string>();
    planItems.forEach((item, idx) => {
      console.log(
        `[getIngredientsForMealPlanRange] Processing planItem ${idx}:`,
      ...
      if (item.recipe_uploads) {
        if (Array.isArray(item.recipe_uploads)) {
          ...
          typeof item.recipe_uploads === 'object' &&
          item.recipe_uploads !== null
        ) {
      ...
      if (item.recipe_id && recipeTitle && typeof recipeTitle === 'string') {
        ...
        console.log(
          `[getIngredientsForMealPlanRange] Added to recipeIdToNameMap: ID=${item.recipe_id}, Title=${recipeTitle}`,
        );
      } else {
        console.warn(
          `[getIngredientsForMealPlanRange] Skipping planItem ${idx} due to missing recipe_id or title. Recipe ID: ${item.recipe_id}, Title: ${recipeTitle}`,
        );
      }
    });
    const uniqueRecipeIds = Array.from(recipeIdToNameMap.keys());
    console.log(
      '[getIngredientsForMealPlanRange] Finished processing planItems. Unique recipe IDs found:',
      JSON.stringify(uniqueRecipeIds, null, 2),
    );

    if (uniqueRecipeIds.length === 0) {
      console.log(
        '[getIngredientsForMealPlanRange] No unique recipe IDs with titles found after processing. Returning empty array.',
      );
      return [];
    }
    ...
    console.log(
      '[getIngredientsForMealPlanRange] Step 3: Fetching details for unique recipe IDs:',
      JSON.stringify(uniqueRecipeIds, null, 2),
    );
    const recipeDetailsPromises = uniqueRecipeIds.map(id =>
      supabase.rpc('get_recipe_details', {
        p_recipe_id: id,
        p_user_id: userId,
      }),
    );
    ...
    recipeDetailsResults.forEach((result, index) => {
      ...
      if (result.error) {
        console.warn(
          `[getIngredientsForMealPlanRange] Error fetching details for recipe ${recipeId}:`,
          result.error.message,
        );
        return; // Skip this recipe if details fetch failed
      }

      const recipeData = Array.isArray(result.data)
        ? result.data[0]
        : result.data;

      if (!recipeData || !Array.isArray(recipeData.ingredients)) {
        console.warn(
          `[getIngredientsForMealPlanRange] No ingredients data or malformed data for recipe ${recipeId}. Data received:`,
          recipeData,
        );
    ...
    console.log(
      '[getIngredientsForMealPlanRange] Aggregated ingredients:',
      JSON.stringify(finalAggregatedList, null, 2),
    );
