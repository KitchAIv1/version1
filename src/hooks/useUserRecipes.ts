...
const fetchUserRecipes = async (userId?: string): Promise<RecipeItem[]> => { 
  console.log('[[USE_USER_RECIPES_HOOK_V2_FETCH_START]] Calling get_profile_details. UserID:', userId);
  if (!userId) { 
    console.error('[[USE_USER_RECIPES_HOOK_V2_FETCH_ERROR]] User not authenticated for get_profile_details');
    throw new Error('User not authenticated for fetching profile details');
  }

  // Switched to call get_profile_details
  const { data, error } = await supabase.rpc('get_profile_details', { p_user_id: userId });

  if (error) {
    console.error('[[USE_USER_RECIPES_HOOK_V2_FETCH_ERROR]] Error fetching profile details:', error);
    throw new Error(`Failed to fetch profile details: ${error.message}`);
  }

  console.log('[[USE_USER_RECIPES_HOOK_V2_FETCH_SUCCESS]] Data received from get_profile_details:', data);
  ...
  if (profileData && Array.isArray(profileData.saved_recipes)) {
    console.log('[[USE_USER_RECIPES_HOOK_V2_FETCH_RETURNING]] from profileData.saved_recipes:', profileData.saved_recipes);
  ...
  console.warn('[[USE_USER_RECIPES_HOOK_V2_FETCH_WARN]] No saved_recipes array found in expected format within get_profile_details response', data);
  console.log('[[USE_USER_RECIPES_HOOK_V2_FETCH_RETURNING]] Empty array (default from get_profile_details attempt)');
  return [];
};
