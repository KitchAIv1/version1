...
export const comparePantryMatches = (
  ...
  const feedPct = feedTotalCount > 0 ? Math.round((feedUserCount / feedTotalCount) * 100) : 0;
  ...
  if (feedShowsZero && recipeDetailShowsMatch) {
    ...
    if (feedTotalCount !== recipeTotalIngredients) {
      possibleCause = 'Different ingredient counts between feed and recipe detail';
    } else if (feedUserCount === 0 && recipeMatchedCount > 0) {
      possibleCause = 'Feed RPC not detecting matched ingredients that recipe detail RPC finds';
    } else if (feedUserCount > 0 && feedPct === 0) {
      possibleCause = 'Rounding issue: feed has matches but percentage rounds to 0';
    }
...
export const logPantryMatchDiscrepancy = (debugInfo: PantryMatchDebugInfo) => {
  if (debugInfo.discrepancy) {
    console.warn(`[PantryMatchDebug] Discrepancy detected for recipe ${debugInfo.recipeId}:`, {
...
export const refreshRecipePantryMatch = async (
  ...
  console.log(`[PantryMatchDebug] Force refreshing pantry match for recipe ${recipeId}`);
  
  // Invalidate specific queries
  await queryClient.invalidateQueries({ queryKey: ['pantryMatch', recipeId, userId] });
  await queryClient.invalidateQueries({ queryKey: ['recipeDetails', recipeId, userId] });
  await queryClient.invalidateQueries({ queryKey: ['feed'] });
  
  // Refetch the data
  await queryClient.refetchQueries({ queryKey: ['pantryMatch', recipeId, userId] });
