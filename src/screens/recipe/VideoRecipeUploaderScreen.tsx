...
const CollapsibleCard: React.FC<{ title: string; children: React.ReactNode; defaultCollapsed?: boolean; icon?: string }> = 
  ({ title, children, defaultCollapsed = false, icon }) => {
  const [isCollapsed, setIsCollapsed] = useState(defaultCollapsed);
  const animatedHeight = useState(new Animated.Value(defaultCollapsed ? 0 : 1))[0];
    ...
    <View style={styles.cardContainer}>
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          {icon && <Feather name={icon as any} size={18} color={isCollapsed ? '#666' : BRAND_PRIMARY} style={{ marginRight: 8 }} />}
          <Text style={[styles.cardTitle, isCollapsed ? {} : styles.activeCardTitle]}>{title}</Text>
        </View>
        <Icon name={isCollapsed ? "expand-more" : "expand-less"} size={24} color={isCollapsed ? '#666' : BRAND_PRIMARY} />
      </TouchableOpacity>
...
const VideoRecipeUploaderScreen: React.FC<VideoRecipeUploaderScreenProps> = ({ navigation: navPropFromProps }) => {
  ...
  const handleRemoveIngredient = (indexToRemove: number) => {
    if (ingredients.length === 1) {
      setIngredients([{ name: '', quantity: '', unit: '' }]); return;
    }
  ...
  const handleRemoveStep = (indexToRemove: number) => {
    if (preparationSteps.length === 1) {
      setPreparationSteps(['']); return;
    }
    setPreparationSteps(preparationSteps.filter((_, index) => index !== indexToRemove));
  };

  const handlePublish = async () => {
    if (!videoUri) { Alert.alert('Validation Error', 'Please select a video.'); return; }
    if (!thumbnailUri) { Alert.alert('Validation Error', 'Please select a thumbnail.'); return; }
    if (!title.trim()) { Alert.alert('Validation Error', 'Please enter a recipe title.'); return; }
    if (!prepTimeMinutes.trim() || !cookTimeMinutes.trim() || !servings.trim()) {
      Alert.alert('Validation Error', 'Please fill in Prep Time, Cook Time, and Servings.'); return;
    }
    if (isNaN(parseInt(prepTimeMinutes)) || isNaN(parseInt(cookTimeMinutes)) || isNaN(parseInt(servings))) {
      Alert.alert('Validation Error', 'Prep Time, Cook Time, and Servings must be valid numbers.'); return;
    }
    if (ingredients.length === 0 || ingredients.every(ing => !ing.name.trim())) {
       Alert.alert('Validation Error', 'Please add at least one valid ingredient (name is required).'); return;
    }
    if (preparationSteps.length === 0 || preparationSteps.every(step => !step.trim())) {
       Alert.alert('Validation Error', 'Please add at least one preparation step.'); return;
    }

    let recipeId;
    try { recipeId = uuidv4(); } catch (e: any) {
      Alert.alert('Error', `Could not generate a unique ID for the recipe: ${e.message || 'Unknown error'}`); return;
    }
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.scrollContentContainer}>
      <Animated.View style={{
        ...
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.screenSubtitle}>Share your culinary masterpiece with the world</Text>
        </View>
          ...
          <View style={styles.mediaPreviewWrapper}>
            {videoUri ? (
              ...
              <View style={[styles.videoPreview, styles.mediaPlaceholder]}>
                ...
                <Text style={styles.mediaPlaceholderText}>Showcase your recipe in action</Text>
              </View>
            )}
            <TouchableOpacity 
              ...
              activeOpacity={0.8}
            >
              <Feather name={videoUri ? "refresh-cw" : "video"} size={18} color={videoUri ? BRAND_PRIMARY : "#fff"} style={{ marginRight: 8 }} />
              <Text style={videoUri ? styles.buttonOutlineText : styles.buttonText}>
          ...
          <View style={styles.mediaPreviewWrapper}>
            {thumbnailUri ? (
              ...
              <View style={[styles.thumbnailPreview, styles.mediaPlaceholder]}>
                ...
                <Text style={styles.mediaPlaceholderText}>Add an appetizing cover image</Text>
              </View>
            )}
            <TouchableOpacity 
              ...
              activeOpacity={0.8}
            >
              <Feather name={thumbnailUri ? "refresh-cw" : "image"} size={18} color={thumbnailUri ? BRAND_PRIMARY : "#fff"} style={{ marginRight: 8 }} />
              <Text style={thumbnailUri ? styles.buttonOutlineText : styles.buttonText}>
        ...
        <View style={styles.formContainer}>
          <CollapsibleCard title="Recipe Details" icon="info" defaultCollapsed={false}>
            <TextInput 
          ...
          <CollapsibleCard title={`Ingredients (${ingredients.filter(i => i.name.trim()).length})`} icon="shopping-bag">
            ...
            <TouchableOpacity 
              ...
              activeOpacity={0.8}
            >
              <Feather name="plus-circle" size={20} color={BRAND_PRIMARY} style={{ marginRight: 8 }} />
              <Text style={styles.addButtonText}>Add Ingredient</Text>
            </TouchableOpacity>
          </CollapsibleCard>

          <CollapsibleCard title={`Preparation Steps (${preparationSteps.filter(s => s.trim()).length})`} icon="list">
          ...
          <CollapsibleCard title="Dietary Tags" icon="tag">
            <Text style={styles.tagSectionLabel}>Help others discover your recipe with relevant tags</Text>
            <View style={styles.tagsContainer}>
              {DIET_TAGS_OPTIONS.map(tag => (
                ...
                >
                  ...
                  <Text style={[styles.tagText, dietTags.includes(tag) && styles.tagTextSelected]}>
                    {tag}
                  </Text>
                </TouchableOpacity>
              ))}
        ...
        >
          {isUploading ? (
            ...
            <View style={styles.publishButtonContent}>
              <Feather name="upload-cloud" size={20} color="#fff" style={{ marginRight: 10 }} />
        ...
        {isUploading && uploadProgress > 0 && (
          <View style={styles.progressBarContainer}>
              ...
              <Text style={styles.progressText}>{`${(uploadProgress * 100).toFixed(0)}%`}</Text>
          </View>
        )}
      </Animated.View>
    </ScrollView>
