...
const parseRecognizedQuantity = (
  recognizedQty: string,
...
export const useStockManager = () => {
  ...
  const handleSaveItem = async (
    ...
    try {
      ...
      if (originalItemName && originalItemName !== itemToSave.item_name) {
        console.log(
          `[useStockManager] Updating item by matching old name: ${originalItemName} to ${itemToSave.item_name}`,
        );
        ...
        console.log(
          '[useStockManager] Upserting new item:',
          itemPayload.item_name,
        );
        response = await supabase
          .from('stock')
      ...
      console.log(
        '[useStockManager] Item saved successfully. Refreshing stock...',
      );
  ...
  const saveScannedItems = async (items: PreparedItem[]): Promise<boolean> => {
    ...
    try {
      ...
      if (upsertDbError) {
        console.error(
          '[useStockManager] Supabase upsert error for scanned items:',
          upsertDbError,
        );
        throw upsertDbError;
      }
      console.log(
        '[useStockManager] Scanned items saved successfully. Refreshing stock...',
      );
      await fetchStock(userId);
      setIsSaving(false);
      return true;
  ...
  const deleteStockItem = async (itemToDelete: StockItem) => {
    if (!userId || (!itemToDelete.id && !itemToDelete.item_name)) {
      ...
      Alert.alert(
        'Error',
        'User session or item identifier not found. Cannot delete item.',
      );
      return false;
    }
    setIsDeleting(true);
    ...
    try {
      ...
      if (itemToDelete.id) {
        console.log(
          `[useStockManager] Deleting item by ID: ${itemToDelete.id}`,
        );
        query = query.eq('id', itemToDelete.id);
      } else {
        console.log(
          `[useStockManager] Deleting item by name: ${itemToDelete.item_name}`,
        );
      ...
      console.log(
        '[useStockManager] Item deleted successfully. Refreshing stock...',
      );
      await fetchStock(userId);
      setIsDeleting(false);
      return true;
  ...
  const recognizeItemsFromImage = async (
    base64Image: string,
    ...
    try {
      ...
      if (!data || !Array.isArray(data.items)) {
        console.error(
          '[useStockManager] Invalid data format from recognition service:',
          data,
        );
        throw new Error('Invalid data format from recognition service.');
      }
      const preparedItems: PreparedItem[] = data.items.map(
        (item: RecognizedItem) => {
      ...
      setRecognitionError(
        error.message || 'Failed to recognize items from image.',
      );
      Alert.alert(
        'Recognition Failed',
  ...
  const handleCaptureAndProcessImage = async (base64Image: string) => {
    ...
    if (recognizedItems && recognizedItems.length > 0) {
      ...
      Alert.alert(
        'No Items Recognized',
        'Could not find any items in the image. Try again?',
      );
    }
  };
