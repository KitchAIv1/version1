-- Fix get_recipe_details RPC to use recipe_uploads table instead of recipes table
-- This fixes the issue where AI recipes saved to recipe_uploads were not being retrieved properly
...
BEGIN
  ...
  FROM recipe_uploads r  -- CRITICAL FIX: Use recipe_uploads instead of recipes
...
COMMENT ON FUNCTION get_recipe_details(UUID, UUID) IS 'Get complete recipe details from recipe_uploads table (where AI recipes are stored) including real-time comment count, like status, and save status for a user'; 