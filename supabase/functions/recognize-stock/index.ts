  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.error("AI response JSON parsing error:", e, "Raw content:", aiResponseContent);
        ...
        if (jsonMatch && jsonMatch[1]) {
          try {
            ...
            console.log("Successfully parsed fallback JSON from markdown block.");
          } catch (fallbackError) {
            console.error("Fallback JSON parsing also failed:", fallbackError);
            throw new Error("Invalid AI response format - not valid JSON, even in fallback.");
          }
        } else {
      ...
      if (!result || typeof result !== 'object' || !Array.isArray(result.items)) {
        console.error("AI response does not match expected structure { items: [...] }");
      ...
      if (validItems.length !== result.items.length) {
        console.warn("Some items in the AI response were invalid and filtered out.", {
      ...
      console.error(`❌ Vision model failed: Status ${errorStatus}, Code: ${errorCode}, Message: ${errorMsg}`);
      
      // If it's a model access error, return a clear message
      if (errorStatus === 403 || errorCode === 'model_not_found' || errorMsg.includes('does not have access')) {
        return new Response(JSON.stringify({
          error: "Vision AI Access Required",
          message: "Your OpenAI account needs access to vision models. Please upgrade your OpenAI plan or contact OpenAI support for access to gpt-4o vision capabilities.",
          items: []
        }), {
